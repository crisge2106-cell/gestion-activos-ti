#!/usr/bin/env node
/**
 * ACTUALIZACIÓN COMPLETA CON PREFIJOS PARA TODOS
 * Genera nombres con prefijos para los 228 equipos basándose en su tipo
 */
...
console.log('\n🔄 Actualización completa con prefijos para TODOS los equipos...\n');
...
try {
  ...
  // Mapeo de 179 equipos conocidos
  ...
  console.log(`📋 Paso 1: Actualizando ${Object.keys(mapeoConocidos).length} equipos con nombres del Excel...\n`);
  ...
  // Paso 2: Generar nombres para equipos sin prefijo específico
  console.log(`🔨 Paso 2: Generando nombres para equipos faltantes...\n`);
  ...
  const equiposFaltantes = db.prepare(`
    SELECT id, tipo FROM equipos
    WHERE nombre LIKE 'Equipo-%' OR nombre IS NULL OR nombre = ''
    ORDER BY tipo, id
  `).all();

  console.log(`   Encontrados ${equiposFaltantes.length} equipos sin prefijo específico\n`);

  for (const eq of equiposFaltantes) {
    let prefijo = 'EQ';
    ...
    const nuevoNombre = `${prefijo}-${String(correlativs[prefijo] || 1).padStart(3, '0')}`;
    correlativs[prefijo] = (correlativs[prefijo] || 0) + 1;

    db.exec(`UPDATE equipos SET nombre = '${nuevoNombre}' WHERE id = '${eq.id}'`);
  ...
  console.log(`📊 Equipos por tipo:`);
  for (const prefijo of prefijos) {
    const count = db.prepare(`SELECT COUNT(*) as count FROM equipos WHERE nombre LIKE '${prefijo}-%'`).get().count;
    if (count > 0) {
