#!/usr/bin/env node
/**
 * CORRECCIÓN NUCLEAR DE SEED.JSON
 *
 * Este script:
 * 1. Lee el seed.json actual
 * 2. Mapea los nombres correctos directamente
 * 3. Reescribe seed.json con los nombres correctos
 * 4. NO depende de la BD (evita problemas de SQLite)
 */
...
console.log('\n' + '='.repeat(70));
console.log('  CORRECCIÓN NUCLEAR DE seed.json');
...
try {
  ...
  console.log(`✅ Leídos ${seed.equipos.length} equipos\n`);

  // Mapeo de serie -> nombre (de Excel)
  const mapeoExcel = {
    '3350P602970': 'IMP-001',
    '3350P603051': 'IMP-010',
  ...
  // Paso 1: Aplicar nombres del mapeo
  console.log('🔄 PASO 1: Aplicando nombres del Excel\n');

  let aplicados = 0;
  for (const eq of seed.equipos) {
    if (eq.serie && mapeoExcel[eq.serie]) {
      eq.nombre = mapeoExcel[eq.serie];
      aplicados++;
    }
  }

  console.log(`✅ ${aplicados} nombres aplicados del Excel\n`);

  // Paso 2: Auto-generar nombres para equipos sin nombre
  console.log('🔨 PASO 2: Generando nombres automáticos\n');
  ...
  for (const eq of seed.equipos) {
    ...
    eq.nombre = `${prefijo}-${numeroFormato}`;

    if (!correlatives[prefijo]) correlatives[prefijo] = numero;
    correlatives[prefijo]++;
    generados++;
  }

  console.log(`✅ ${generados} nombres generados automáticamente\n`);

  // Paso 3: Crear backup
  console.log('💾 PASO 3: Crear backup\n');
  ...
  // Paso 4: Guardar seed.json actualizado
  console.log('💾 PASO 4: Guardando seed.json\n');

  fs.writeFileSync(seedPath, JSON.stringify(seed, null, 2));
  console.log(`✅ seed.json guardado\n`);
  ...
  for (const eq of seed.equipos) {
    if (eq.nombre) {
      const pref = eq.nombre.split('-')[0];
      resumen[pref] = (resumen[pref] || 0) + 1;
    }
  }

  for (const [pref, count] of Object.entries(resumen).sort()) {
  ...
  console.log('=' .repeat(70));
  console.log('✅ ¡CORRECCIÓN NUCLEAR COMPLETADA!');
  console.log('=' .repeat(70));
  console.log();
  console.log('📝 Próximos pasos:');
  console.log('   1. Elimina activos.db (la BD será regenerada desde seed.json)');
  console.log('      DEL activos.db');
  console.log('   2. Reinicia el servidor:');
  console.log('      iniciar_servidor.bat');
  console.log('   3. Abre http://localhost:3335');
  console.log('   4. Presiona Ctrl+F5');
  console.log('   5. Ve a Inventario\n');
